 ...
 * ver 0.85, Radu ION, 08.02.2012: added identicalStrings to TextProcessing.
 * 
*/
...
namespace PexaccSim
{
    ...
    public class Measure
    {
        ...
        //    m.createOctaveData("pexacc-neg", "en", "train.neg.en", "hr", "train.neg.hr", 0);
    ...
    public class Resources
    {
        ...
        Dictionary<string, float> readGIZAPPDict(string rfile)
        {
            ...
            while ((line = DICT.ReadLine()) != null)
            {
                ...
                if (toks.Length != 3)
                {
                    Console.Error.WriteLine("PexaccSim::Resources::readGIZAPPDict['" + rfile + "']: line " + linecnt + " is not well formed...");
                    continue;
                }
                ...
                try
                {
                    score = System.Convert.ToSingle(toks[2], CultureInfo.InvariantCulture);
                }
        ...
        public string lemmatizeSrcWordUnique(string word)
        {
            //For function words, return the word as it is.
            if (isSrcStopWord(word))
            {
            ...
            Dictionary<string, int> lemmas = lemmatizeWord(word, srcInflections);
            int minlen = 1000;
            string shortestlemma = "";

            foreach (string l in lemmas.Keys.ToArray())
            {
                if (lemmas[l] < minlen)
                {
                    minlen = lemmas[l];
            ...
            return shortestlemma;
        }

        public string lemmatizeTrgWordUnique(string word)
        {
            //For function words, return the word as it is.
            if (isTrgStopWord(word))
            {
            ...
            Dictionary<string, int> lemmas = lemmatizeWord(word, trgInflections);
            int minlen = 1000;
            string shortestlemma = "";
    ...
    {
        //ready
        public static string SENTENCESPLITMARKER = "#CUT#";
        ...
        {
            Regex ssrx = new Regex(@"((?:\p{Ll}|\p{N})\s*(?:\p{Pe}|\p{Pf}|""|')?\s*[.?!]+?\s*)((?:\p{Ps}|\p{Pi}|""|')?\s*\p{Lu}(?:(?:\p{Lu}|\p{Ll}|\p{N})+|\s))");
            Match m = ssrx.Match(text);

            while (m.Success)
            {
                text = text.Replace(m.Value, m.Groups[1].Value + TextProcessing.SENTENCESPLITMARKER + m.Groups[2].Value);
                m = m.NextMatch();
            } //end all sentences.

            string[] sentences = Regex.Split(text, TextProcessing.SENTENCESPLITMARKER);
        ...
        public static bool identicalStrings(string stext, string ttext)
        {
            stext = stext.ToLower();
            ttext = ttext.ToLower();
            ...
            stext = Regex.Replace(stext, @"\W+|\s+", "");
            ttext = Regex.Replace(ttext, @"\W+|\s+", "");

            if (TextProcessing.similarity(stext, ttext) > 0.95)
            {
                return true;
            }
